...
/// Tool for finding redundant public declarations
struct FindRedundantPublicTool {
    static let name = "find_redundant_public"
    static let description = "Identify public declarations that could be internal"
    ...
    static func execute(arguments: [String: Any]) async -> String {
        do {
            ...
            // Parse and filter results for public/open declarations only
            let allResults = try OutputParser.parseResults(output)
            let publicResults = OutputParser.filterRedundantPublic(allResults)

            let scanOutput = ScanOutput.success(results: publicResults)
